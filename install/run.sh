...
# 删除空行（谨慎操作，建议提前备份）
sed -i.bak '/^$/d' "$listFile" || error_exit "清理list.txt失败" 11
...
main() {
    ...
    for item in "${items[@]}"; do
        server_num=$item
        app_port=$((app_port_start + app_port_index * 10))
        
        echo "当前正在配置：行号=$line_num | IP=$current_ip | 端口=$app_port | 编号=$server_num"
        
        # 生成配置文件
        export current_ip app_port server_num
        ...
        ansible-playbook -i "${current_ip}," \
            ...
            -e "role_name=game" \
            example.yaml || error_exit "Ansible任务失败，编号: $server_num" 14
